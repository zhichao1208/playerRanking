...
playerList << getVoted(mid).each{|p|p["mid"]=mid}.each{|p|p["matchday"]=mdid}
...
lastMatch = Match.new(@lan,player["mid"],league["cid"],league["sid"])

lastMatchInfo = lastMatch.show

goalInfo = lastMatch.goals.find_all{|goal| goal["player"]["id"].to_i == player["id"].to_i}.count

player["hasGoals"] = goalInfo
player["lastMatchInfo"] = lastMatchInfo
...
return trendingPlayers =  bestOfLast.sort_by{|p| p["last/Avg"]}.reverse.map{|p|
 ...
 {"name"=>p["playerInfo"]["data"]["info"]["name"],
  ...
  "raisingSpeed"=>p["votesAvgRate"].round(2).to_s,
...
[bl17,pl17,laliga17,sa17].each do |league|
